...
*** [30/05/25]
*  Termino FetchController y UploadController

*** [01/06/25]
*  Conectado feed con backend y añadido soporte para el logout y filtros de los vídeos.


*** [02/06/25]
*  Añadida ruta checkUser para validar el usuario
        Ya que en el frontend solo puedo mirarlo por la cookie 'user' que cualquiera puede cambiar, pero para cosas como subir un vídeo o detectar si el perfil X es el perfil de un usuario necesito algo más fiable.
*  Creado componente Profile
        Además se han creado un servicio de usuarios y una interfaz que los represente
*  Aplicados cambios en la base de datos
        He repasado la db y si los followers y suscribers se apuntan en tablas para la relacion M:M no hace falta mantener una cuenta ni en users ni en stream_details. Si quiero saber a quien estas suscrito lo miro por tu id en suscribers_streamers y viceversa
*  Componente Profile


        [] Le falta terminar parte del diseño y crear un botón de empezar directo solo cuando el profileUser sea el mismo que el usuario que hay ahora (usar checkUser).

        [✔] PONLE A LAS TABS OVERFLOW HIDDEN POR DIOS ️
        [] haz que el boton de seguir funcione y cambie de 'seguir' a 'dejar de seguir' tendrá que ser opcional ( no va a salir el botón si es tu perfil )

        [] crea una tab de contenido premium con un dialog de suscribe.

        [] añade en la db en usuario una descripcion, atributo en el modelo y read y update (no hagas create, pon default "" en la db)

*** [05/06/25]
*  Componente profile
        [x] haz que el boton de seguir funcione y cambie de 'seguir' a 'dejar de seguir' tendrá que ser opcional ( no va a salir el botón si es tu perfil )
        [x] Le falta terminar parte del diseño y crear un botón de empezar directo solo cuando el profileUser sea el mismo que el usuario que hay ahora (usar checkUser).

        IMPORTANTE:
        Apunta en el tfg que has usado fontsource para iconos faciles de implementar.

        Mas cosas en el issue
*** [06/06/25]
*  Terminar el perfil
        Termino lo que hay en el issue

        Me falta añadir pa subir streams

*** [07/06/25]
*  Correccion errores perfil
        La base de datos tenia claves foraneas que apuntaban a la id del streamer_details en follower_streamer y suscriber_streamer
        Pero como se puede seguir a cualquier usuario debe de apuntar a la id de users.

        Además había ciertas variables mal asignadas y para poder actualizar la informacion dependiendo del perfil que fuera, Es decir
        si vas del perfil de alguien a otro perfil y no has cambiado de componente, hay que meter esa logica en el suscribe de ActiveRoute.params, que
        actualiza cada vez que cambia la url.
*  Añadir lo que falta en el perfil
        NOTA PARA apuntar en el tfg y en el README: No se si añadiré subida de archivos para las miniaturas ya que hacerlo seguro es muy dificil y
        tienen más prioridad funcionalidades más necesarias como comentarios. Para los que tendré que hacer un poller y explicar en mi tfg que no voy a
        implementar ws porque no es necesario para tan pocos usuarios y me llevaría mucho tiempo. AÑADIR AL PDF COMO FUNCIONALIDADES A MEJORAR.


*  Añadido favicon, Logo y hero section de la landing.

*  Añadir descripcion a los usuarios y conectarla con el perfil (da la opcion de cambiarla).
        Ya que hago esto y me meto en el backend hago el crud de los comentarios y a lo mejor el de las notificaciones
        Además corrijo el backend, ya que no necesito un follows ni suscribed en la tabla de usuario, he optado por hacerlo en sus propias tablas M:M

*  Panel de subir vídeo con titulo, categoria y desc para enviar, que una vez que se pasen los datos te da la info para empezar directo
        El panel le falta la descripción y que funcione, no pilla el json ;(((
        tengo miedo

*** [08/06/25]
    El json no lo pillaba porque @RequestBody de Mono<..> es rarillo y consume el objeto antes de parsearlo, por eso me decia no requestbody found.

    NOTA IMPORTANTE PARA AÑADIR AL PDF DEL TFG: No añado validacion compleja para desasignarle un canal (caducarlo) a alguien que no empiece directo al tiempo porque para
    un prototipo me parece innecesario.

    Poner en el tfg que la pagina se inspirado en el diseño de yt y hbo sports (max) entre otros.
*  Correcion rapida busqueda feed ahora que hay mas de un directo y copia pega a subscriptions
        Corregido y añadida logica en el logo para variar el a donnde te lleva.
        falta copia pega subs y videos.
